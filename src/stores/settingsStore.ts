...
const DEFAULT_SETTINGS: AppSettings = {
  ...
  creative: {
    ...
    promptTemplates: {
      ...
      rewriteZh: '你是 AI 视频提示词工程师。把用户的粗略输入标准化为视频模型可用的纯中文画面描述。必须使用 100% 纯中文！严禁包含英文单词或英汉混排。',
      rewriteEn: 'You are an AI video prompt engineer. Standardize user rough input into accurate English video prompts suitable for AI models.',
      extractZh: '你是剧本分析师。从给定文本中提取结构化的角色/场景/道具信息。所有输出的描述文本必须使用 100% 纯中文！严禁包含英文单词或英汉混排。',
      extractEn: 'You are a script analyzer. Extract structured character, scene, and prop metadata from the text in clear English.',
      storyboardZh: '你是影视分镜师 + AI 视频提示词工程师，把小说段落重写成 AI 视频模型能直接生成的精确纯中文 prompt。必须包含：(1) 场景环境 (2) 角色外貌服饰 (3) 具体动作 (4) 镜头视角 (5) 画面光影与氛围。',
      storyboardEn: 'You are a film storyboard director and AI prompt engineer. Rewrite novel excerpts into detailed English video prompts including scene setting, character appearance, concrete action, camera angle, lighting, and mood.',
