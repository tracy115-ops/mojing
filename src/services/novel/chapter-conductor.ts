...
export async function magnifyOutlineToBeats(
  ...
  const request: LLMGenerateRequest = {
    taskType: 'planning',
    systemPrompt: `你是一个叙事节拍规划师。将章节大纲拆分为 3-6 个叙事节拍(beat)。

每个 beat 必须指定：
- "description": 节拍内容描述（具体场景/事件，30-60字）
- "targetWords": 目标字数（整数，所有beat总和必须等于${targetWords}）
...
function buildFallbackBeats(outline: string, targetWords: number, chapterIndex: number): Beat[] {
  const patterns: Beat[][] = [
    // Chapter 1: hook + intro + action + suspense
    [
      { index: 0, description: '开篇钩子：用强烈的场景或冲突抓住读者', targetWords: Math.round(targetWords * 0.2), focus: 'hook' as BeatFocus, sceneGoal: '抓住注意力' },
      { index: 1, description: '主角出场与环境建立', targetWords: Math.round(targetWords * 0.25), focus: 'character_intro' as BeatFocus, sceneGoal: '建立主角形象' },
      { index: 2, description: '核心事件展开', targetWords: Math.round(targetWords * 0.3), focus: 'action' as BeatFocus, sceneGoal: '推进主线' },
      { index: 3, description: '悬念收束，制造继续阅读的动力', targetWords: Math.round(targetWords * 0.25), focus: 'suspense' as BeatFocus, sceneGoal: '制造悬念' },
    ],
    // Chapter 2: sensory + suspense
    [
      { index: 0, description: '承接上章悬念，建立新场景', targetWords: Math.round(targetWords * 0.2), focus: 'sensory' as BeatFocus, sceneGoal: '场景过渡' },
      { index: 1, description: '事件推进与角色互动', targetWords: Math.round(targetWords * 0.3), focus: 'dialogue' as BeatFocus, sceneGoal: '深化角色关系' },
      { index: 2, description: '冲突升级', targetWords: Math.round(targetWords * 0.3), focus: 'action' as BeatFocus, sceneGoal: '提升张力' },
      { index: 3, description: '新的悬念或转折', targetWords: Math.round(targetWords * 0.2), focus: 'suspense' as BeatFocus, sceneGoal: '悬念收尾' },
    ],
  ];

  // Default: 起承转合
  const defaultPattern: Beat[] = [
    { index: 0, description: '起：场景建立与承接', targetWords: Math.round(targetWords * 0.2), focus: 'narration' as BeatFocus, sceneGoal: '建立场景' },
    { index: 1, description: '承：事件推进与发展', targetWords: Math.round(targetWords * 0.3), focus: 'action' as BeatFocus, sceneGoal: '推进剧情' },
    { index: 2, description: '转：冲突或转折出现', targetWords: Math.round(targetWords * 0.3), focus: 'suspense' as BeatFocus, sceneGoal: '制造转折' },
    { index: 3, description: '合：收束与悬念', targetWords: Math.round(targetWords * 0.2), focus: 'emotion' as BeatFocus, sceneGoal: '情感落点' },
  ];
